...
<body>
	...
	<c:set var="ps" value="${requestScope.ps}" />
			...
			<center>
				...
				<table width="80%" border="1">
					...
					<tr>
						...
						<a href="board_list.ok?cp=${cp}&ps=${ps}">목록가기</a> |
						<a href="board_edit.ok?idx=${idx}&cp=${cp}&ps=${ps}">편집</a>	|
						<a href="board_delete.ok?idx=${idx}&cp=${cp}&ps=${ps}">삭제</a> |
						<a href="board_rewrite.ok?idx=${idx}&cp=${cp}&ps=${ps}&subject=${board.subject}>">답글</a>
						</td>
					</tr>
				</table>
				<!--  꼬리글 달기 테이블 -->
				<form name="reply" action="board_reply.reply" method="POST">
						<!-- hidden 태그  값을 숨겨서 처리  -->
				...
				<table width="80%" border="1">
					...
					<c:forEach var="reply" items="${requestScope.replyList}">
							<tr align="left">
								...
								<td width="20%">
								<form action="boardreply_delete.reply" method="POST" name="replyDel">
