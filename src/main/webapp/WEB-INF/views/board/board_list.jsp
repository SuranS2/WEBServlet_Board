			...
			<table width="80%" border="1" cellspacing="0" align="center">
				<tr>
					<td colspan="5">
						<!--  
							...
							>> http://192.168.0.29:8090/WebServlet_92_Board_Model2_Mvc/BoardList.ok?ps=10				
						-->
						<form name="list" >
						 pagesize설정: 
				...
				<tr>
					...
					-->
						...
						<!--이전 링크 --> 
						<c:if test="${cp > 1}">
							<a href="board_list.ok?cp=${cp-1}&ps=${ps}">이전</a>
						</c:if>
						<!-- page 목록 나열하기 -->
						<c:forEach var="i" begin="1" end="${pagecount}" step="1">
							<c:choose>
								...
								<c:otherwise>
									<a href="board_list.ok?cp=${i}&ps=${ps}">[${i}]</a>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<!--다음 링크 --> 
						<c:if test="${cp < pagecount}">
							<a href="board_list.ok?cp=${cp+1}&ps=${ps}">다음</a>
						</c:if>
					</td>
